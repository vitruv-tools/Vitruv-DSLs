...
public class CreatedFatherReaction extends AbstractReaction {
  ...
  public CreatedFatherReaction(final Function<ReactionExecutionState, RoutinesFacade> routinesFacadeGenerator) {
  ...
  private static class Call extends AbstractRoutine.Update {
    ...
    public void updateModels(final ReplaceSingleValuedEReference replaceChange, final Family affectedEObject, final EReference affectedFeature, final Member oldValue, final Member newValue, @Extension final FamiliesToInsuranceRoutinesFacade _routinesFacade) {
      _routinesFacade.updateNameAndCorrespondencesOfCorrespondingInsuranceClient(newValue, affectedEObject, Gender.MALE);
      _routinesFacade.tryCreateInsuranceClientFromNewMember(newValue, affectedEObject, Gender.MALE);
    }
  }
  ...
  public void executeReaction(final EChange change, final ReactionExecutionState executionState, final RoutinesFacade routinesFacadeUntyped) {
    ...
    new mir.reactions.familiesToInsurance.CreatedFatherReaction.Call(executionState).updateModels(replaceChange, affectedEObject, affectedFeature, oldValue, newValue, routinesFacade);
  }
}
