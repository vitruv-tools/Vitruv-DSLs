...
public class ChangedFirstNameReaction extends AbstractReaction {
  private ReplaceSingleValuedEAttribute<Member, String> replaceChange;

  public ChangedFirstNameReaction(final Function<ReactionExecutionState, RoutinesFacade> routinesFacadeGenerator) {
  ...
  private static class Call extends AbstractRoutine.Update {
    ...
    public void updateModels(final ReplaceSingleValuedEAttribute replaceChange, final Member affectedEObject, final EAttribute affectedFeature, final String oldValue, final String newValue, @Extension final FamiliesToInsuranceRoutinesFacade _routinesFacade) {
      _routinesFacade.updateInsuranceClientName(affectedEObject);
    }
  }
  ...
  public boolean isCurrentChangeMatchingTrigger(final EChange change) {
    ...
    ReplaceSingleValuedEAttribute<edu.kit.ipd.sdq.metamodels.families.Member, java.lang.String> _localTypedChange = (ReplaceSingleValuedEAttribute<edu.kit.ipd.sdq.metamodels.families.Member, java.lang.String>) change;
    if (!(_localTypedChange.getAffectedElement() instanceof edu.kit.ipd.sdq.metamodels.families.Member)) {
    	return false;
    }
    if (!_localTypedChange.getAffectedFeature().getName().equals("firstName")) {
    	return false;
    }
    ...
    this.replaceChange = (ReplaceSingleValuedEAttribute<edu.kit.ipd.sdq.metamodels.families.Member, java.lang.String>) change;
    return true;
  }
  ...
  public void executeReaction(final EChange change, final ReactionExecutionState executionState, final RoutinesFacade routinesFacadeUntyped) {
    ...
    edu.kit.ipd.sdq.metamodels.families.Member affectedEObject = (edu.kit.ipd.sdq.metamodels.families.Member)replaceChange.getAffectedElement();
    ...
    new mir.reactions.familiesToInsurance.ChangedFirstNameReaction.Call(executionState).updateModels(replaceChange, affectedEObject, affectedFeature, oldValue, newValue, routinesFacade);
  }
}
