...
public class InsertExistingMemberIntoUserChosenFamilyAsChildRoutine extends AbstractRoutine {
  ...
  private static class Match extends AbstractRoutine.Match {
    ...
    public InsertExistingMemberIntoUserChosenFamilyAsChildRoutine.Match.RetrievedValues match(final InsuranceClient insuranceClient) throws IOException {
      ...
      return new mir.routines.insuranceToFamilies.InsertExistingMemberIntoUserChosenFamilyAsChildRoutine.Match.RetrievedValues(insuranceDatabase, correspondingMember);
    }
  }
  ...
  private static class Update extends AbstractRoutine.Update {
    ...
    public void updateModels(final InsuranceClient insuranceClient, final FamilyRegister insuranceDatabase, final Member correspondingMember, @Extension final InsuranceToFamiliesRoutinesFacade _routinesFacade) {
      final Iterable<Family> matchingFamilies = IterableExtensions.<Family>filter(insuranceDatabase.getFamilies(), ((Function1<? super Family, Boolean>)InsuranceToFamiliesHelper.sameLastName(insuranceClient)));
      ...
      if ((chosenFamily == null)) {
        _routinesFacade.insertExistingMemberIntoNewFamilyAsChild(insuranceClient);
      } else {
        _routinesFacade.insertExistingMemberIntoExistingFamilyAsChild(insuranceClient, chosenFamily);
      }
    }
  }

  public InsertExistingMemberIntoUserChosenFamilyAsChildRoutine(final InsuranceToFamiliesRoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final InsuranceClient insuranceClient) {
    super(routinesFacade, reactionExecutionState, calledBy);
    this.inputValues = new InsertExistingMemberIntoUserChosenFamilyAsChildRoutine.InputValues(insuranceClient);
  }
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    retrievedValues = new mir.routines.insuranceToFamilies.InsertExistingMemberIntoUserChosenFamilyAsChildRoutine.Match(getExecutionState()).match(inputValues.insuranceClient);
    if (retrievedValues == null) {
    	return false;
    }
    // This execution step is empty
    new mir.routines.insuranceToFamilies.InsertExistingMemberIntoUserChosenFamilyAsChildRoutine.Update(getExecutionState()).updateModels(inputValues.insuranceClient, retrievedValues.insuranceDatabase, retrievedValues.correspondingMember, getRoutinesFacade());
    return true;
  }
}
