...
public class CreateParentRoutine extends AbstractRoutine {
  ...
  private static class Match extends AbstractRoutine.Match {
    ...
    public CreateParentRoutine.Match.RetrievedValues match(final InsuranceClient insertedClient) throws IOException {
      ...
      return new mir.routines.insuranceToFamilies.CreateParentRoutine.Match.RetrievedValues(insuranceDatabase);
    }
  }
  ...
  private static class Update extends AbstractRoutine.Update {
    ...
    public void updateModels(final InsuranceClient insertedClient, final FamilyRegister insuranceDatabase, @Extension final InsuranceToFamiliesRoutinesFacade _routinesFacade) {
      ...
      if ((familyToInsertInto == null)) {
        _routinesFacade.createParentInNewFamily(insertedClient);
      } else {
        _routinesFacade.createParentInExistingFamily(insertedClient, familyToInsertInto);
      }
    }
  }

  public CreateParentRoutine(final InsuranceToFamiliesRoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final InsuranceClient insertedClient) {
    super(routinesFacade, reactionExecutionState, calledBy);
    this.inputValues = new CreateParentRoutine.InputValues(insertedClient);
  }
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    retrievedValues = new mir.routines.insuranceToFamilies.CreateParentRoutine.Match(getExecutionState()).match(inputValues.insertedClient);
    if (retrievedValues == null) {
    	return false;
    }
    // This execution step is empty
    new mir.routines.insuranceToFamilies.CreateParentRoutine.Update(getExecutionState()).updateModels(inputValues.insertedClient, retrievedValues.insuranceDatabase, getRoutinesFacade());
    return true;
  }
}
