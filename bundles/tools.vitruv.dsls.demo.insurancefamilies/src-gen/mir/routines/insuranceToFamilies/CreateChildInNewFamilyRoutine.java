...
public class CreateChildInNewFamilyRoutine extends AbstractRoutine {
  ...
  private static class Match extends AbstractRoutine.Match {
    ...
    public CreateChildInNewFamilyRoutine.Match.RetrievedValues match(final InsuranceClient insertedClient) throws IOException {
      ...
      return new mir.routines.insuranceToFamilies.CreateChildInNewFamilyRoutine.Match.RetrievedValues(insuranceDatabase);
    }
  }
  ...
  private static class Update extends AbstractRoutine.Update {
    ...
    public void updateModels(final InsuranceClient insertedClient, final FamilyRegister insuranceDatabase, final Family newFamily, @Extension final InsuranceToFamiliesRoutinesFacade _routinesFacade) {
      ...
      _routinesFacade.createChildInExistingFamily(insertedClient, newFamily);
    }
  }

  public CreateChildInNewFamilyRoutine(final InsuranceToFamiliesRoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final InsuranceClient insertedClient) {
    super(routinesFacade, reactionExecutionState, calledBy);
    this.inputValues = new CreateChildInNewFamilyRoutine.InputValues(insertedClient);
  }
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    retrievedValues = new mir.routines.insuranceToFamilies.CreateChildInNewFamilyRoutine.Match(getExecutionState()).match(inputValues.insertedClient);
    if (retrievedValues == null) {
    	return false;
    }
    createdValues = new mir.routines.insuranceToFamilies.CreateChildInNewFamilyRoutine.Create(getExecutionState()).createElements();
    new mir.routines.insuranceToFamilies.CreateChildInNewFamilyRoutine.Update(getExecutionState()).updateModels(inputValues.insertedClient, retrievedValues.insuranceDatabase, createdValues.newFamily, getRoutinesFacade());
    return true;
  }
}
