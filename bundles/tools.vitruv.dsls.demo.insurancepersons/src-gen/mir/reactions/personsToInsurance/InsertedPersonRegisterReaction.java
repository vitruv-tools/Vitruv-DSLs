...
public class InsertedPersonRegisterReaction extends AbstractReaction {
  ...
  private static class Call extends AbstractRoutine.Update {
    ...
    public void updateModels(final InsertRootEObject insertChange, final PersonRegister newValue, final int index, @Extension final PersonsToInsuranceRoutinesFacade _routinesFacade) {
  ...
  public void executeReaction(final EChange change, final ReactionExecutionState executionState, final RoutinesFacade routinesFacadeUntyped) {
    ...
    new mir.reactions.personsToInsurance.InsertedPersonRegisterReaction.Call(executionState).updateModels(insertChange, newValue, index, routinesFacade);
  }
}
