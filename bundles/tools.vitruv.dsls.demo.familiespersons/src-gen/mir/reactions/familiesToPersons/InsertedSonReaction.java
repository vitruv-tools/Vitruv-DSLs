...
public class InsertedSonReaction extends AbstractReaction {
  private InsertEReference<EObject> insertChange;

  public InsertedSonReaction(final Function<ReactionExecutionState, RoutinesFacade> routinesFacadeGenerator) {
  ...
  private static class Call extends AbstractRoutine.Update {
    ...
    public void updateModels(final InsertEReference insertChange, final Family affectedEObject, final EReference affectedFeature, final Member newValue, final int index, @Extension final FamiliesToPersonsRoutinesFacade _routinesFacade) {
      _routinesFacade.createOrFindMale(newValue, affectedEObject);
      _routinesFacade.existingMemberBecameSon_updateNameAndCorrespondencesOfCorrespondingMale(newValue, affectedEObject);
    }
  }
  ...
  public void executeReaction(final EChange change, final ReactionExecutionState executionState, final RoutinesFacade routinesFacadeUntyped) {
    ...
    new mir.reactions.familiesToPersons.InsertedSonReaction.Call(executionState).updateModels(insertChange, affectedEObject, affectedFeature, newValue, index, routinesFacade);
  }
}
