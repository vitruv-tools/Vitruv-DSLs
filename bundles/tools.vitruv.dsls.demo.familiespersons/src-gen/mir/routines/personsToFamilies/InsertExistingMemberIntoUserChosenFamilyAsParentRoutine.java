...
public class InsertExistingMemberIntoUserChosenFamilyAsParentRoutine extends AbstractRoutine {
  ...
  private static class Match extends AbstractRoutine.Match {
    ...
    public InsertExistingMemberIntoUserChosenFamilyAsParentRoutine.Match.RetrievedValues match(final Person renamedPerson) throws IOException {
      ...
      return new mir.routines.personsToFamilies.InsertExistingMemberIntoUserChosenFamilyAsParentRoutine.Match.RetrievedValues(familiesRegister, correspondingMember);
    }
  }
  ...
  private static class Update extends AbstractRoutine.Update {
    ...
    public void updateModels(final Person renamedPerson, final FamilyRegister familiesRegister, final Member correspondingMember, @Extension final PersonsToFamiliesRoutinesFacade _routinesFacade) {
      final Iterable<Family> matchingFamilies = IterableExtensions.<Family>filter(IterableExtensions.<Family>filter(familiesRegister.getFamilies(), ((Function1<? super Family, Boolean>)PersonsToFamiliesHelper.sameLastname(renamedPerson))), 
        ((Function1<? super Family, Boolean>)PersonsToFamiliesHelper.noParent(renamedPerson)));
      ...
      if ((chosenFamily == null)) {
        _routinesFacade.insertExistingMemberIntoNewFamilyAsParent(renamedPerson);
      } else {
        _routinesFacade.insertExistingMemberIntoExistingFamilyAsParent(renamedPerson, chosenFamily);
      }
    }
  }

  public InsertExistingMemberIntoUserChosenFamilyAsParentRoutine(final PersonsToFamiliesRoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final Person renamedPerson) {
    super(routinesFacade, reactionExecutionState, calledBy);
    this.inputValues = new InsertExistingMemberIntoUserChosenFamilyAsParentRoutine.InputValues(renamedPerson);
  }
  ...
  protected boolean executeRoutine() throws IOException {
    ...
    retrievedValues = new mir.routines.personsToFamilies.InsertExistingMemberIntoUserChosenFamilyAsParentRoutine.Match(getExecutionState()).match(inputValues.renamedPerson);
    if (retrievedValues == null) {
    	return false;
    }
    // This execution step is empty
    new mir.routines.personsToFamilies.InsertExistingMemberIntoUserChosenFamilyAsParentRoutine.Update(getExecutionState()).updateModels(inputValues.renamedPerson, retrievedValues.familiesRegister, retrievedValues.correspondingMember, getRoutinesFacade());
    return true;
  }
}
