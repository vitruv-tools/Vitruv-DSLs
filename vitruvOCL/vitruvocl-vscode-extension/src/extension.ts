...
async function applyThemeOnFirstInstall(context: vscode.ExtensionContext): Promise<void> {
    ...
    const answer = await vscode.window.showInformationMessage(
        'VitruvOCL: Apply the included "MultiModelOCL Dark" theme for optimized syntax highlighting?',
        'Yes', 'No'
    );
    if (answer === 'Yes') {
    ...
    try {
        ...
        if (instanceFiles.length === 0 || !instanceFiles.some(f => f.endsWith('.correspondence'))) {
            vscode.window.showErrorMessage(
                'No VSUM found. Run VSUMExample.main() from your IDE to create one, then try again.'
            );
            return;
        }
 ...
 */
function collectVsumFiles(vsumFolder: string): string[] {
    if (!fs.existsSync(vsumFolder) || !fs.statSync(vsumFolder).isDirectory()) return [];
    const result: string[] = [];
    ...
    function walk(dir: string) {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (entry.isFile()) {
                ...
                if (VSUM_SKIP_DIRS.has(entry.name)) continue;
                walk(fullPath);
            }
        }
    }
...
async function findInstanceFiles(constraintFileUri?: vscode.Uri): Promise<string[]> {
    ...
    const config = vscode.workspace.getConfiguration('vitruvocl');
    const configuredPath: string | undefined = config.get('vsumPath');
    if (configuredPath) {
    ...
    if (files1.length > 0) return files1;
    ...
    const broader = await vscode.workspace.findFiles(
        '{**/vsumtest/**,**/vsumexample/**,**/vsum-storage/**}',
        '**/target/**'
    );
    if (broader.length > 0) return broader.map(u => u.fsPath);
