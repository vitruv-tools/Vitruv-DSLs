 ...
 * 
 * Core OCL# Principle: "Everything is a collection"
 * 
 * All values are represented as lists of elements:
 * - Singleton: [elem] (¡τ! - exactly 1 element)
 * - Empty: [] (no τ - "kein Wert")
 * - Multi-valued: [e1, e2, e3] (collections)
 * 
 * The Type's Ctype properties determine how the list is interpreted:
 * - {τ} (Set): Unique, unordered
 * - [τ] (Sequence): Non-unique, ordered
 * - {{τ}} (Bag): Non-unique, unordered
 * - ⟨τ⟩ (OrderedSet): Unique, ordered
 */
public class Value {
    ...
    /**
     * Creates a Value from a list of elements.
     * This is the main constructor for OCL# values.
     */
     ...
     * If value is already List<OCLElement>, use it directly.
     * Otherwise, wrap it as a singleton OCLElement.
     */
    ...
    /**
     * In OCL#, null doesn't exist - only empty collections.
     ...
     * 
     * Behavior depends on Type's Ctype properties:
     * - Set (unique, unordered): Union with uniqueness
     * - Bag (non-unique, unordered): Concatenation with duplicates
     * - Sequence (non-unique, ordered): Concatenation preserving order
     * - OrderedSet (unique, ordered): Union preserving order
     */
     ...
     */
    public boolean includes(OCLElement element) {
        return containsElement(elements, element);
    }
    ...
    /**
     * Returns a new Value with element included.
     * Immutable - does not modify this Value.
     */
    ...
    /**
     * Returns a new Value with element excluded.
     * - Set/OrderedSet: Removes all occurrences
     ...
     * Returns empty if this is empty (OCL# semantics).
     */
    public Value first() {
        if (isEmpty()) {
    ...
    /**
     * Returns element at index (1-based OCL indexing).
     * Only valid for ordered collections.
     */
    public Value at(int index) {
        if (!runtimeType.isOrdered()) {
            throw new UnsupportedOperationException(
                    "at() requires an ordered collection");
        }
    ...
    /**
     * Returns the index of first occurrence (1-based).
    ...
    public Value reverse() {
        if (!runtimeType.isOrdered()) {
            throw new UnsupportedOperationException(
                    "reverse() requires an ordered collection");
        }
    ...
    /**
     * Normalizes this Value according to its Ctype.
     * Unordered collections (Set, Bag) are sorted for canonical form.
     */
    ...
    /**
     * Compares two Values for ordering (used in nested collections).
     * Required by OCLElement.compare() for NestedCollection comparison.
     */
     ...
     * 
     * Example: Set{Set{1,2}, Set{3,4}}.flatten() → Set{1,2,3,4}
     * 
