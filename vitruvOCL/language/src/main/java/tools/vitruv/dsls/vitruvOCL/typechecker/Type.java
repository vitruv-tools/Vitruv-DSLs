 ...
 * 
 * Extended to support OCL# multiplicity semantics:
 * - Primitive types (INTEGER, STRING, BOOLEAN) have implicit SINGLETON
 * multiplicity
 * - Collections created via factory methods (set(), sequence(), optional())
 * - Metamodel types (EClass references) for cross-metamodel constraints
 * 
...
public abstract class Type {
     ...
     * 
     * For explicit singleton wrapping (usually not needed as primitives are already
     * singletons)
     * 
     ...
     * 
     * Default: SINGLETON for all primitive types (INTEGER, STRING, BOOLEAN)
     * Override: CollectionType returns its explicit multiplicity
     * 
     ...
     * 
     * Default: this (for primitive types, the element type is themselves)
     * Override: CollectionType returns its wrapped element type
     * 
     ...
     * 
     * Used for cross-metamodel consistency checking with the ~ operator.
     */
    private static class MetaclassType extends Type {
        ...
        public boolean isConformantTo(Type other) {
            ...
            if (other instanceof MetaclassType otherMeta) {
                ...
                return otherMeta.eClass.isSuperTypeOf(this.eClass) ||
                        this.eClass.equals(otherMeta.eClass);
            }
     ...
     * 
     * Represents OCL# collection types:
     * - Set{T}
     * - Sequence[T]
     * - Bag{{T}}
     * - OrderedSet<T>
     ...
     * In OCL#, this corresponds to μ = u (unique).
     */
     ...
     * 
     * Used in type inference for expressions with multiple branches (e.g.,
     * if-then-else).
     * 
     * @param t1 First type
     ...
     * This is a helper for OCL# semantic comparison.
     */
    public static int compare(tools.vitruv.dsls.vitruvOCL.evaluator.Value v1,
        ...
        for (int i = 0; i < elems1.size(); i++) {
            int elemCompare = tools.vitruv.dsls.vitruvOCL.evaluator.OCLElement.compare(
                    elems1.get(i),
                    elems2.get(i));
            if (elemCompare != 0) {
                return elemCompare;
