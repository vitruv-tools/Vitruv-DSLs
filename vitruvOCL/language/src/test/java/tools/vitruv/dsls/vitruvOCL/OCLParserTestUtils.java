 ...
 *   ↓
 * Lexer                (Tokenization - OCLLexer)
 *   ↓
 * CommonTokenStream    (Token buffer)
 *   ↓
 * Parser               (Parse tree construction - OCLParser)
 *   ↓
 ...
 * <pre>{@code
 * // Parse from string
 * ParseTree tree = OCLParserTestUtils.parseString(
 *     "context spaceMission::Spacecraft inv: self.mass > 0"
 * );
 *
 * // Parse from file
 * ParseTree tree = OCLParserTestUtils.parseFile(
 *     "test-inputs/valid/simple.ocl"
 * );
 *
 * // Get readable tree representation
 * String treeString = OCLParserTestUtils.treeToString(tree, parser);
 * }</pre>
 *
 ...
 */
public class OCLParserTestUtils {
   ...
   * ParseTree tree = OCLParserTestUtils.parseString(constraint);
   * }</pre>
   *
   ...
   *
   * <ol>
   *   <li><b>Lexical analysis:</b> {@link OCLLexer} tokenizes the input character stream
   ...
   *   <li><b>Syntax analysis:</b> {@link OCLParser} constructs parse tree using {@code
