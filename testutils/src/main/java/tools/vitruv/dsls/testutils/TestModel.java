...
/**
 * A model that can be used in tests to encapsulate access to root elements of test model.
 *
 * @param <T> the root element type
 */
public interface TestModel<T extends EObject> {
  /**
   * Returns the root element type of this model.
   *
  ...
  /**
   * Persists the given object at the given {@link URI} and adds it as a model root.
   *
   ...
   * root object of the model, otherwise an {@link IllegalStateException} is thrown.
   *
   * @param object the object to persist, must not be {@code null}
