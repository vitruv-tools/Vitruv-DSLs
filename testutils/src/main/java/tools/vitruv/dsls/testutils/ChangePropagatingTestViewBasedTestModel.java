...
/**
 * A {@link TestModel} that propagates performed changes using the functionality of an underlying
 * {@link TestView}.
 *
 * @param <T> the model's root element type
 */
public class ChangePropagatingTestViewBasedTestModel<T extends EObject>
    implements TestViewBasedTestModel<T> {
  ...
  /**
   * Instantiates a new {@link ChangePropagatingTestViewBasedTestModel}.
   *
   * @param testView the underlying {@link TestView} to use for change propagation
   * @param rootElementType the root element type of the model
   */
  public ChangePropagatingTestViewBasedTestModel(
      NonTransactionalTestView testView, Class<T> rootElementType) {
  ...
  public Collection<EObject> getRootObjects() {
    return FluentIterable.from(rootResourceURIs)
        .filter(uri -> uri != null && URIUtil.existsResourceAtUri(uri))
        .transformAndConcat(uri -> testView.resourceAt(uri).getContents())
        .toSet();
  }
  ...
  public void applyChanges(
    ...
    FluentIterable.from(changes)
        ...
        .filter(otherTestModelToUpdate.getRootElementType())
        .forEach(
            object -> {
