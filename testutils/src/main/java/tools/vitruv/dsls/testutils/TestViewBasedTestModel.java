...
public interface TestViewBasedTestModel<T extends EObject> extends TestModel<T> {
	...
	/**
	 * Applies the given modification function to this test model. Updates the other
	 * given test model with additions of root elements to that model
	 * 
	 * @param modelModificationFunction the function with modifications to apply
	 * @param otherTestModelToUpdate    a test model to update upon creation of new
	 *                                  root objects of that model
	 */
	void applyChanges(Consumer<TestModel<T>> modelModificationFunction,
