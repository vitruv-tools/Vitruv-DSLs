...
public class JavaFileGeneratorTest {
    ...
    void testGenerateSimple(){
            ...
            assertEquals(expectedResult, result,"only package should be printed");

            result = JavaFileGenerator.generateClass("implementation","test",null);
            expectedResult = "package test;\n\nimplementation";
            assertEquals(expectedResult, result,"implementation is not printed");
    }
    ...
    void testGenerateImportHelper(){
        ...
        assertEquals(expectedResult.toString(),generateClassWithImportHelper(importHelper),"Empty Import failed");

        importHelper.typeRef(JavaFileGenerator.class);
        expectedResult.append("import ").append(JavaFileGenerator.class.getName()).append(";\n");
        assertEquals(expectedResult+"\n\n",generateClassWithImportHelper(importHelper),"normal Import failed");

        importHelper.typeRef(JavaFileGenerator.class.getName());
        assertEquals(expectedResult+"\n\n",generateClassWithImportHelper(importHelper),"duplicate import not deduplicated");
        ...
        assertEquals(expectedResult +"\n\n",generateClassWithImportHelper(importHelper),"Static Import failed");
    ...
    void testGetJavaFilePath(){
        ...
        assertEquals(expectedResult,result, "Simple test failed");

        GenericClassNameGenerator nameGenerator = new GenericClassNameGenerator("package","class");
        assertEquals(expectedResult,JavaFileGenerator.getJavaFilePath(nameGenerator), "Name Generator does not work");

        expectedResult = "package1"+JavaFileGenerator.FSA_SEPARATOR+"package2"+JavaFileGenerator.FSA_SEPARATOR+"class.java";
        result = JavaFileGenerator.getJavaFilePath("package1.package2.class");
        assertEquals(expectedResult,result, "several directories does not work");
    }
